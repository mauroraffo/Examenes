...
function renderEquipo(eq){
  ...
  // Equipo last event
  // Equipo last event - corregido
  let latestDate = null;
  let maxHoro = null;
  for(const r of rows){
    const d = parseDate(r[COL_FECHA]);
    if(d && (!latestDate || d > latestDate)) latestDate = d;

    const h = parseFloat(r[COL_HORO]);
    if(!isNaN(h) && (maxHoro === null || h > maxHoro)) maxHoro = h;
  }
...
function buildForm(eq, last){
  ...
  const preset = GRID_PRESETS[GRID_PRESET] || GRID_PRESETS["Scoop / Auto"];
  const layout = preset.layout;
      ...
      for(const cell of row){
        ...
        if(!cell){
          ...
          div.innerHTML = `
            ...
            <label>Serie</label><input type="text" id="serie_${p}" value="${serie}">
            <label>Rem. Izq</label><input type="text" id="rizq_${p}" value="${rizq}">
            <label>Rem. Der</label><input type="text" id="rder_${p}" value="${rder}">
            <label>CT examen</label><input type="text" id="ct_${p}" value="${ct}">
            <label>Presión PSI</label><input type="text" id="psi_${p}" value="${psi}">
            <label>Fotos</label><input type="file" id="fotos_${p}" accept="image/*" multiple capture="environment">
...
// Save draft
$("#btnSave").addEventListener('click', ()=>{ console.log('BTN GUARDAR EJECUTADO');
  ...
  const rows = {};
  for(const p of positionsList()){
    const serieEl = $("#serie_"+p);
    const rizqEl = $("#rizq_"+p);
    const rderEl = $("#rder_"+p);
    const ctEl = $("#ct_"+p);
    const psiEl = $("#psi_"+p);

    rows[p] = {
      serie: serieEl ? serieEl.value.trim() : '',
      rizq:  rizqEl ? rizqEl.value.trim() : '',
      rder:  rderEl ? rderEl.value.trim() : '',
      ct:    ctEl ? ctEl.value.trim() : '',
      psi:   psiEl ? psiEl.value.trim() : ''
    };
  }
